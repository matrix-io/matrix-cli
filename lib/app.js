...
program
  .command('install <app>')
  .description('Usage: install -a app1, install -s sensor1')
  .action(function(app) {
      ...
      Matrix.api.app.deploy({
        ...
        resp.on('end', function() {
          ...
          Matrix.api.app.install(deployInfo, Matrix.deviceId, function(err, resp) {
            if (err) { return console.error('App Install Fail'.red, err); }
            console.log('App Installed'.green, appName, '--->', Matrix.deviceId, resp);
            endIt();
          })
        })
      })
