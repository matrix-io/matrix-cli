...
async.series([
  ...
  function () { //Main flow
    ...
    program
      ...
      .action(function () {
        ...
        async.series([
          Matrix.validate.userAsync,
          Matrix.validate.deviceAsync,
          async.apply(Matrix.api.app.trigger, Matrix.config.devices[0].identifier, 'amazing-matrix-ping')
        ], function (err) {
          if (err) {
            console.error(err.message.red);
            debug('Error:', err);
    ...
    if (process.argv.length === 2 || commandList.indexOf(process.argv[2]) === -1) {
      ...
      if (!_.isEmpty(Matrix.config.groupName)) {
        log.push(t('matrix.group_title').grey + ':'.grey, Matrix.config.groupName);
      } else if (!_.isEmpty(Matrix.config.devices) && Matrix.config.devices.length === 1) {
        log.push(t('matrix.device_title').grey + ':'.grey, Matrix.config.devices[0].identifier);
      } else {
