            ...
            if (_.isUndefined(app)) {
              ...
              Matrix.api.app.start(app, Matrix.config.device.identifier, function (err, res) {
                ...
                Matrix.firebase.app.watchStatus(appId, function (status) {
                  Matrix.loader.stop()
                  console.log('-------start end status---->',status);
                  if(status =='active'){
    ...
    .action(function (app, options) {
      ...
      Matrix.firebaseInit(function () {
        Matrix.firebase.app.getIDForName( app, function(err, appId){
          ...
          debug('appId>', appId);
          Matrix.firebase.app.getStatus(appId, function (status) {
            console.log('-------stop start status---->',status);
            debug("Status Watch: " + Matrix.config.user.id + '>' + Matrix.config.device.identifier + '>' + appId + '>' + status);
            ...
            if (_.isUndefined(app)) {
              ...
              Matrix.firebase.app.watchStatus(appId, function (status) {
                Matrix.loader.stop();
                console.log('-------stop end status---->',status);
                if(status =='active'){
                    console.log(t('matrix.stop.stop_app_successfully') + ':', app);
                }
                endIt();
              });
              console.log('-------->', Matrix.config.device.identifier, '****', app);
        ...
        Matrix.firebase.app.getIDForName( app, function(err, appId){
          ...
          Matrix.firebase.app.getStatus(appId, function (status) {
            ...
            if (_.isUndefined(app)) {
              ...
              console.log(t('matrix.restart.restart_app_status_error') + ':', app , ' (' + err.message.red + ')');
              endIt();
            } else {
