...
Matrix.localization.init(Matrix.localesFolder, Matrix.config.locale, function () {
  ...
  program.command( 'login', i('matrix.help_login').grey );
  program.command( 'logout' )
    .description(i('matrix.help_logout'))
    .action( function () {
      Matrix.helpers.removeConfig( function ( err ) {
        console.log( i('matrix.logout.logout_success').green );
      } );
    } )
  ...
  program.command( 'use', i('matrix.help_use').grey );
  program.command( 'set [target]', i('matrix.help_set') + '\n[env <environment>, config <app> k=v, locale <locale>]'.grey )

  program.command( 'Management' )
  program.command( 'sim [target]', i('matrix.help_sim') + '\n[ init, restore, start, stop, save, clear ]' );
  program.command( 'list [target]', i('matrix.help_list') + '\n[ apps, devices, all, groups ] '.grey )

  /** Update the App or the Matrix **/
  program
    .command( 'reboot' )
    .description( i('matrix.help_reboot') )
    .action( function () {
      Matrix.api.device.reboot( Matrix.config.device.identifier, function ( err ) {
        console.log( i('matrix.reboot.rebooting') + '...' );
        process.exit();
      } );
    } );
  ...
  program.command( 'search', i('matrix.help_search').grey );
  program.command( 'install', i('matrix.help_install') + '\n[ app / sensor ] <name>. '.grey + i('matrix.help_install_default').grey)
  program.command( 'config', i('matrix.help_config') )
  ...
  program
    ...
    .description( i('matrix.help_uninstall'), ' <app>' )
    .action( function ( name ) {
      Matrix.api.app.uninstall( name, Matrix.config.device.identifier, function ( err, resp ) {
        if ( err ) console.error( err );
        console.log( i('matrix.uninstall.uninstalled') + ' ', name + '...' );
        process.exit();
      } );
    } );

    program.command( 'update', i('matrix.help_update') + '\n<appName> <version>'.grey )

  program
    .command( 'start [app]' )
    .description( i('matrix.help_start') )
    .option( '-a, --all' )
    .action( function ( app, options ) {
      ...
      console.log( i('matrix.start.starting_app') + ': ', app, Matrix.config.device.identifier );
      Matrix.api.app.start( app, Matrix.config.device.identifier, function ( err, res ) {
        if ( err ) return console.log( err );
        console.log( i('matrix.start.start_app_successfully') + ': ', app );
        endIt();
      } );
    } );
  //
  program
    .command( 'stop [app]' )
    .description( i('matrix.help_stop') )
    .option( '-a, --all' )
    .action( function ( app, options ) {
      console.log( i('matrix.stop.stopping_app') + ': ', app );
      if ( options.all ) {
        //FIXME: hacky, passes this to server
        app = 'all-applications'
      }
      Matrix.api.app.stop( app, Matrix.config.device.identifier, function ( err ) {
        console.log( i('matrix.stop.stop_app_successfully') + ':', app );
        endIt();
      } );
    } );
  //
  program
    .command( 'restart <app>' )
    .description( i('matrix.help_restart') )
    .action( function ( app ) {
      console.log( i('matrix.restart.restarting_app') + ': ', app );
      Matrix.api.app.restart( app, function ( err, res ) {
        console.log( i('matrix.restart.restart_app_successfully') + ':', app, res );
        endIt();
      } );
    } );
  ...
  //
  /** App management **/
  program.command( 'create <app>', i('matrix.help_create') )
    //
    // // Run from current directory
  program.command( 'deploy <dir>', i('matrix.help_deploy') )
    //
    // program
  ...
  program
    ...
    .description( i('matrix.help_trigger') )
  ...
  if ( process.argv.length === 2 || commandList.indexOf( process.argv[ 2 ] ) === -1 ) {
    ...
    log.push( i('matrix.api_title').grey + ': '.grey + Matrix.options.apiUrl, i('matrix.streaming_title').grey + ': '.grey + Matrix.options.mxssUrl );


    if ( !_.isEmpty( Matrix.config.user ) ) {
      log.push( '\n' + i('matrix.user_title').grey + ':'.grey, Matrix.config.user.username );
    }
    if ( !_.isEmpty( Matrix.config.device ) ) {
      log.push( i('matrix.device_title').grey + ':'.grey, Matrix.config.device.identifier );
    }

    log.push( i('matrix.locale_title').grey + ':'.grey, Matrix.config.locale );
