...
program
  .command('set-config <app> [key=value]')
  .description('Options: [info, config, apps] once using a device.')
...
if (!process.argv.slice(2).length) {
  ...
  console.log('_  _ ____ ___ ____ _ _  _')
  console.log('|\\/| |__|  |  |__/ |  \\/   _ |','[o]'.grey)
  console.log('|  | |  |  |  |  \\ | _/\\_ |_ |_ |', 'v'+Matrix.version.grey)
...
var help2 = help.forEach(function(l, i) {
  if (i > 5) {
    ...
    var pad = 30 - command.length;
    console.log(_.repeat(' ', pad), command.grey, '-', desc);
  }
});
console.log('\n\n')
