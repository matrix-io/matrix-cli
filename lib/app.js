...
program
  ...
  .action(function(dir){
      ...
      _.each( files, function(file){
        zip.append(fs.createReadStream(dir +'/'+ file), { name: file });
      });
      ...
      function onEnd() {
        console.log('Packed!');
        Matrix.api.app.deploy({ appConfig: configObject, file: tmp, name: dir }, function(err, resp){
          if (err) console.error(err);
          ...
          resp.on('end', function(){
            ...
            Matrix.api.app.install( deployInfo, Matrix.deviceId, function(err, resp){
              if (err) { console.error('App Install Fail'.red, err);}
