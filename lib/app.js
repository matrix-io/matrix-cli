...
.action(function appDeploy(appName) {
  var pwd = process.cwd();
...
.description('Runs a local instance of MatrixOS')
...
.action(function(options){
  ...
  //test for docker
  try {
    var proc = require('child_process').execSync('which docker', { stdio: [null,null,null]});
  } catch (e) {
  ...
  var proc = require('child_process').exec('docker run ' + [
    ...
    'admobilize/matrix-os'].join(' '), {}, function(err, out, stderr){
...
var commandList = _.map(program.commands, function(p) {
  ...
  if (process.argv.length === 2 || commandList.indexOf(process.argv[2]) === -1) {
    ...
    console.log(_.repeat(' ', 10), '|  | |  |  |  |  \\ | _/\\_ |_ |_ |', 'v'.grey + Matrix.version.grey)
