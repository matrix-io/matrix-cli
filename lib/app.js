...
program
  .command('use <cmd>')
  .description('Options: [name || identifier] to indicate device.')
  .option('-h --help', 'Never mind')
  .action(function(cmd) {
...
program
  .command('list <cmd>')
  .description('Options: [device, group, config, apps] once using a device.')
  .action(function(cmd) {
    if (cmd === 'device'){
      listDevice();
    } else if (cmd === 'group'){
    ...
    if (cmd === 'apps') {
      Matrix.api.app.list(function(err, resp) {
        ...
        if (_.isUndefined(resp)){
          console.warn('No Apps Installed for User');
        }
...
if (!process.argv.slice(2).length) {
  ...
  var cakeIndex = helps.length - 31;
  var cake = helps.slice(cakeIndex, helps.length);
  var help = helps.slice(0, cakeIndex);
  ...
  var log = [ 'API'.grey , options.apiUrl];
...
var help2 = help.forEach(function(l, i) {
  if (i > 5) {
