...
function trackEvent(eventName, extras, cb) {
  ...
  // FIXME: track proxy is broke, lets disable this
  if (true || !process.env.hasOwnProperty('TRACKOK')) {
...
function saveRawDeviceInfo(deviceId, deviceSecret) {
  const filepath = require('os').homedir() + '/.matrix/deviceRegistration.txt';
  const content = deviceId + ' ' + deviceSecret + '\n';
...
module.exports = {
  ...
  saveRawDeviceInfo: saveRawDeviceInfo
};