...
function lookupAppId(name, cb){
  // config
  if (  Matrix.config.hasOwnProperty('appMap') === true ) {
    return cb( _.findKey( Matrix.config.appMap, { name : name } ))
  }
  // if no, firebase
  Matrix.firebase.user.getIDForName(name, cb);
}
...
function displayDevices(devices) {
  ...
  _.forEach(devices, function (device, deviceId) {
    ...
    table.push([ deviceId, device.name || '', status, moment.unix(device.lastSeen, "YYYYMMDD").fromNow()] || '-' );
  });
...
function displaySearch(raw, needle) {
  ...
  _.each(_.values(raw), function (app) {
    ...
    table.push([app.name, app.shortName || '', app.description || '']);
  })
  ...
  require('inquirer').prompt(prompts).ui.process.subscribe(function (ans) {
    ...
    if (!_.isNull(ans.name.match(/sensors|integrations|events|services/)) && _.isArray(ans.answer)) {
      ...
      _.each(ans.answer, function (answer) {
        policy[ans.name][answer] = true;
      })
    }
  }, function (e) { console.error(e) }, function () {

    // in case there is no policy
    if ( _.isEmpty(policy) ){
      policy = {};
    }
