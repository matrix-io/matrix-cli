...
function getConfig() {
  ...
  // does the dir write?
  try {
    fs.openSync(__dirname + '/../store.json', 'w');
  } catch (e) {
    debug(e);

    if (e.code === 'EACCES'){
      var configPath = require('path').normalize(__dirname + '/../');
      console.log('Configuration not writable. Ensure permissions are set.');
      console.log('\nsudo chmod 777 '.grey+ configPath.grey);
      console.log();
      process.exit(1);
    }
