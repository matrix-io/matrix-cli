...
function checkConfigWrite(){
  fs.openSync(__dirname + '/../store.json', 'w', function(err){
    if (err){
...
function getConfig() {
  ...
  try {
    ...
    debug(t('matrix.helpers.config_write_error'), e);
    var configPath = require('path').normalize(__dirname + '/../store.json');
    console.log('Configuration file not writable. Ensure writable permissions are set for', configPath);
  }
