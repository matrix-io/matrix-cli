  ...
  function stepThroughConfig() {
    ...
    _.forIn(policy, function (v, k) {
      if (!_.isEmpty(v)) {
        ...
        debug('Skipping empty object ' + k + ':' + v);
      }
    });
...
function prettyPrint(obj, indent) {
  ...
  for (var property in obj) {
    ...
    if (typeof value == 'string') {
      value = "'" + value + "'";
      value = value.yellow; // Color leafs in yellow
    } else if (typeof value == 'object') {
      if (value instanceof Array) {
        value = "[\n" + prettyPrint(value, indent + tab) + "\n" + indent + "]";
      } else {
        var od = prettyPrint(value, indent + tab);
        value = "{\n" + od + "\n" + indent + "}";
      }
    } else {
      value = value.toString().yellow; // Color leafs in yellow
    }
    result += indent + "'" + property.cyan + "' : " + value + ",\n"; //Key(in cyan) : Value
  }
...
function installApp(details, callback) {
  ...
  console.log('Please specify your application access permissions:'.yellow);
