...
function displayDevices(devices) {
  ...
  _.forEach(devices, function (device, deviceId) {
    ...
    if (status) {
      if(currentDevice) status = 'ok'.cyan;  
      else status = 'ok'.green;
    } else {
      if(currentDevice) status = 'no'.yellow; 
      else status = 'no'.red;
    }
    ...
    if (currentDevice){ 
      table.push([ deviceId.green, device.name.green || '', device.description.green || '', status, time] );
    } else {
      table.push([ deviceId, device.name || '', device.description || '', status, time] );
    }
