...
function displayGroups(groups) {
  ...
  _.forEach(groups, function (item) {
    item.devices = item.devices.map(d => lookupDeviceName(d));
    table.push([item.name, item.devices.join(', '), _.size(item.devices)]);
  });
...
function displayApps(apps) {
  ...
  var table = new Table({
    chars: { 'mid': '', 'left-mid': '', 'mid-mid': '', 'right-mid': '' },
    head: ['Name'.underline, 'Device'.underline, 'v'.underline, 'Description'.underline, 'Status'.underline],
    colWidths: [15, 14, 7, 34 + ttyExtra, 10]
  });

  _.forEach(apps, function (item) {
    var deviceName = lookupDeviceName(item.deviceId);
    if (deviceName === 'no device name') {
      deviceName = item.deviceId;
    }
    table.push([item.name,  deviceName || '', item.version || '', item.description || '', item.status || ''])
  });
...
function installSetup(appId, versionId, policy, callback) {
  ...
  var progress, installTimer, didTimeout;
  progress = {};
  installTimer = {};
  didTimeout = {};
  Matrix.loader.start();

  async.map(Matrix.config.devices, (device, done) => {
    var deviceId = device.identifier;
    var deviceName = lookupDeviceName(deviceId);
    progress[deviceId] = false;
    ...
    Matrix.firebase.app.install(Matrix.config.user.token, deviceId, appId, versionId, policy, {
      error: function (err) {
        ...
        if (err && err.hasOwnProperty('details') && err.details.hasOwnProperty('error')) {
          ...
          console.error('\n' + deviceName.green+ ': ' + t('matrix.install.app_install_error').red + ': ', err);
        }
        done(null);
      },
      finished: function () {
        ...
        Matrix.firebase.deviceapps.watchForNewApp(deviceId, appId, function (app) {
          ...
          if (_.has(app, 'runtime.status') && app.runtime.status != 'pending') {
            ...
            clearTimeout(installTimer[deviceId]);
            if (status === 'error') {
              error = new Error(deviceName.green + ": " + t('matrix.install.app_install_error'), ' ', app);
              console.log(error.message);
              return done(null, { [deviceName]: error.message});
            } else if (status === 'inactive') {
              //good
              installCounter++;
              console.log('\n'+deviceName.green+": "+t('matrix.install.app_install_success').green+' ('+installCounter+'/'+Matrix.config.devices.length+')');
              return done(null);
            } else if (status === 'active') {
              error = new Error(deviceName.green + ": " + 'App running already, will attempt restart after reinstall.');
              console.log(error.message);
              return done(null, { [deviceName]: error.message});
            } else if (didTimeout[deviceId]) {
              return done(null);
            } else {
              error = new Error(deviceName.green + ": " + 'Unknown status ' + status);
              console.log(error.message);
              return done(null, { [deviceName]: error.message});
            }
          }
      ...
      progress: function (msg) {
        ...
        if (!progress[deviceId]) {
          progress = true;
          Matrix.loader.stop();
          process.stdout.write('\n'+deviceName.green + ": " + t('matrix.install.progress') + ':' + msg);
        } else {
          process.stdout.write('\n'+deviceName.green + ": " + '.' + msg);
        }
      }
    });
  }, (err, results) => {
    results = results.filter(res => !_.isUndefined(res));
    if (!_.isEmpty(results)) return callback(results);
      ...
      Matrix.firebaseInit(function () {
        ...
        Matrix.helpers.refreshDeviceMap(function () {
          ...
          if (Matrix.config.keepDevice && _.has(Matrix.config.keepDevice, Matrix.config.user.id)) {
            ...
            async.each(Matrix.config.keepDevice[Matrix.config.user.id], (device, callback) => {
              //Get the device name
              var deviceName = device.name;
              //Get the device token
              Matrix.api.device.register(device.identifier, function (err, state) {
                ...
                if (state.status === "OK") {
                  ...
                  Matrix.config.devices.push({
                    ...
                    token: status.results.device_token
                  });
                }
              ...
              Matrix.helpers.saveConfig(function () {
                if (Matrix.config.devices.length > 1) {
                  console.log('Using group ' + Matrix.config.groupName);
                }
                Matrix.config.devices.forEach(device => {
                  console.log('Using device:'.grey, device.name, 'ID:'.grey, device.identifier);
                });
                return cb();
              });
            });
