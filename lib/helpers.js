...
function trackEvent(eventName, extras, cb){
  ...
  debug('Track Event'.yellow, eventName, extras, process.env);
  ...
  request.post({
    ...
    if(JSON.parse(r.body).created !== true){
...
var profile = {
  ...
  prompt: function (currentData, callback) {
    console.log('CURR PROFILE: ', currentData);
    var nameP = {
      name: 'name',
      description: 'Name',
      required: true,
    ...
    var repoP = {
      ...
      default: currentData.githubUrl || ''
    }

    console.log('currentData.trackOk === true', currentData.trackOk, ' _ ',  currentData.trackOk === true);    
    var trackP = {
      name: 'trackOk',
  ...
  update: function (profile, callback) {
    console.log('UPDATE WITH: ', profile);
    var updateError;
    var events = {
      error: function (err) {
        Matrix.loader.stop();        
