...
const installTimeoutSeconds = 30; //seconds to get a install response from device
...
function trackEvent(eventName, extras, cb) {
  ...
  request.post({
    url: 'https://keen-proxy.admobilize.com',
    headers: {
    ...
    try {
      ...
      var body = '(No body)';
      if (_.has(r, 'body')) body = r.body;
      debug('Error parsing server response: ', body, '\n Error: \n', err);
    }
    if (created !== true) {
...
function syncSaveConfig() {
  var err;
  try { fs.writeFileSync(configFile, JSON.stringify(Matrix.config)); } catch (error) { err = error; }
      ...
      if (!err) {
        fs.writeFile(path, JSON.stringify(content, null, 2), function (err) {
          if (err) {
            ...
            //TODO Delete package.json backup?
          }
          debug('Package file updated ==>', content);
          if (_.isFunction(cb)) {
            cb();
...
function lookupAppId(deviceId, name, cb) {
  if (Matrix.config.hasOwnProperty('deviceMap') === true && Matrix.config.deviceMap.hasOwnProperty(deviceId)) {
    debug('Local app lookup: ');
    var device = Matrix.config.deviceMap[deviceId];
    ...
    if (!_.isUndefined(results)) {
      ...
      return Matrix.firebase.app.getIDForName(deviceId, name, cb);
    }
...
function displayGroups(groups) {
  if (_.isEmpty(groups)) return t('matrix.list.no_results').red;

  var table = new Table({
    chars: { 'mid': '', 'left-mid': '', 'mid-mid': '', 'right-mid': '' },
    head: ['Name'.underline, 'Devices'.underline, 'Device Num'.underline],
    colWidths: [18, 55 + ttyExtra, 12]
  });
  
  _.forEach(groups, function (devices, key) {
    devices = devices.map(d => lookupDeviceName(d));
    table.push([key, devices.join(', '), _.size(devices)]);
  });
...
function installSetup(deviceId, appId, versionId, policy, callback) {
  var deviceName = lookupDeviceName(deviceId);

  debug("\n"+deviceName.green+': '+"Installing to device... ")

  var progressTimeoutSeconds = 60;
  var progressTimeout;
  var progress;
  var error;
  var didTimeout = false;
  Matrix.firebase.app.install(Matrix.config.user.token, deviceId, appId, versionId, policy, {
    error: function (err) {
      if (err && err.hasOwnProperty('details') && err.details.hasOwnProperty('error')) {
        error = new Error('\n' +deviceName.green+': '+ t('matrix.install.app_install_error').red + ': ' + err.details.error);
      } else {
        error = new Error('\n' +deviceName.green+': '+ t('matrix.install.app_install_error').red + ': ' + err.message);
      }
      return callback(error);
    },
    finished: function () {
      clearTimeout(progressTimeout);
      debug(deviceName.green+': '+'install finished');
      // Watch for the app on the user record
      Matrix.firebase.deviceapps.watchForNewApp(deviceId, appId, function (app) {
        debug(deviceName.green+': '+'new app>', app)
        if (_.has(app, 'runtime.status') && app.runtime.status != 'pending') {
          var status = app.runtime.status;
          debug(deviceName.green+': '+'status>', status)
          ...
          if (status === 'error') {
            ...
            error = new Error('App running already, will attempt restart after reinstall.');
          } else {
      ...
      console.log('\n' + deviceName.green+': '+ t('matrix.install.waiting_for_device_install').green + '...'.green);
    ...
    start: function () {
      debug(deviceName.green+': '+'start install')
      _.once(function () {
        console.log(deviceName.green+': '+ t('matrix.install.start') + '...')
      });
    },
    progress: function (msg) {
      ...
      if (!progress) {
        progress = true;
        console.log('\n'+deviceName.green+': '+ t('matrix.install.progress') + '...');

        progressTimeout = setTimeout(() => {
          console.log(deviceName.green+': '+'Install timed out. Try again later.')
          didTimeout = true;
          return callback(null);
...
function installApp(deviceId, details, callback) {
  ...
  if (details.hasOwnProperty('skipPolicyCheck') && details.skipPolicyCheck === true) {
    ...
    installSetup(deviceId, details.id, details.versionId, policy, callback);
  } else {
...
function login(userOptions, cb) {
  ...
  if (_.has(userOptions, 'trackOk')) {
    if (_.isBoolean(userOptions.trackOk)) Matrix.config.trackOk[userOptions.username] = userOptions.trackOk;
    else Matrix.config.trackOk[userOptions.username] = (userOptions.trackOk.toLowerCase() === 'y') ? true : false;
  }
  ...
  Matrix.api.auth.client(Matrix.options, function (err, out) {
    ...
    Matrix.api.auth.user(Matrix.config.user, function (err, state) {
      ...
      if (!_.isEmpty(state.refresh_token)) Matrix.config.user.refreshToken = state.refresh_token;
  ...
  request.get(url, function (error, response, body) { //Get the upload URL
    ...
    if (error) {
      ...
      if (!body.hasOwnProperty(error) && body.hasOwnProperty('status') &&
        ...
        debug('Install Response:', body);
        error = new Error(t('matrix.deploy.app_install_failed').red);
      }
    }
...
function syncRefreshToken(refreshToken) {
  var done = false;
  var result;
  Matrix.api.user.refresh(refreshToken, function (err, token) {
    result = { err: err, token: token };
    done = true;
  });
...
function storeDevice(deviceId, deviceToken) {
  const deviceName = lookupDeviceName(deviceId);

  // store in group
  if (!_.isUndefined(Matrix.config.group)) {

    if (!Matrix.config.group.hasOwnProperty('devices')) {
      Matrix.config.group.devices = [];
    }

    Matrix.config.group.devices.push({
      identifier: deviceId,
      token: deviceToken
    });

    //Create the object for keep device after session expired
    if (!Matrix.config.keepDevices) Matrix.config.keepDevices = {};
    
    //Create key for the current user into the object for keep device after session expired
    if (!_.has(Matrix.config.keepDevices, Matrix.config.user.id)) {
      Matrix.config.keepDevices[Matrix.config.user.id] = [];
    }
    //Put the data into the object for keep device after session expired
    Matrix.config.keepDevices[Matrix.config.user.id].push({
      identifier: deviceId,
      name: deviceName
    });
  }
    ...
    Matrix.config.device = {}
    Matrix.config.device.identifier = deviceId;
    Matrix.config.device.token = deviceToken;
    ...
    //Put the data into the object for keep device after session expired
    Matrix.config.keepDevice[Matrix.config.user.id].identifier = deviceId;
    Matrix.config.keepDevice[Matrix.config.user.id].name = deviceName; 
  }
}

function registerDeviceAsync(deviceId, cb) {
  // still API dependent, TODO: depreciate to firebase
  Matrix.api.device.register(deviceId, function(err, state) {
    if (err) {
      debug('Error:', deviceId, err);
      return cb(err);
    }
    if (state.status === 'OK') {
      const deviceName = lookupDeviceName(deviceId);
    
      storeDevice(deviceId, state.results.device_token);

      return cb(null);
    } else {
      debug('Matrix Use Error Object:', deviceId, state);
      let err; 
      if (state.error === 'access_token not valid.') {
        err = new Error(t('matrix.use.not_authorized').red + '\n' + t('matrix.use.invalid_token')+'. '+t('matrix.use.try').grey+' matrix login');
      } else {
        err = new Error('Error ' + state.status_code.red + state.error);
      }
      return cb(err);
    }
...
// register an array of deviceIds
function registerDevicesAsync(deviceIds, cb) {
  async.each(deviceIds, (did, done) => {
    registerDeviceAsync(did, done);
  }, cb);
}
...
function getUserGroups(cb) {
  Matrix.firebase.user.getUserGroups(function(err, groups) {
    ...
    // make all groups arrays of valid devices
    groups = Object.keys(groups).reduce((acc, name) => {
      if (!_.isArray(groups[name])) {
       acc[name] = []; 
      } else acc[name] = groups[name].filter(did => did && !_.isEmpty(did));
      return acc;
    }, {});
...
function getDevicesFromGroup(groupName, cb) {
  Matrix.firebase.user.getDevicesFromGroup(groupName, (err, deviceIds) => {
    if (err) {
      debug('Error: ',err);
      return cb(err);
    }
    ...
    deviceIds = deviceIds.filter(did => !_.isEmpty(did) && did !== false);
...
module.exports = {
  ...
  getDevicesFromGroup: getDevicesFromGroup,
  ...
  profile: profile,
  refreshToken: Matrix.api.user.refresh,
  removeConfig: removeConfig,
  registerDeviceAsync: registerDeviceAsync,
  registerDevicesAsync: registerDevicesAsync,
  saveConfig: saveConfig,
  syncSaveConfig: syncSaveConfig,
