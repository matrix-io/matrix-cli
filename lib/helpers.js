...
function logout(cb) {
  ...
  if(Matrix.config.device){
    console.log('-----1-------->',  Matrix.config.device.identifier);
    ...
    Matrix.config.keepDevice[ Matrix.config.user.id].name = Matrix.helpers.lookupDeviceName(Matrix.config.device.identifier)
  }

  Matrix.config = _.omit(Matrix.config, keysToRemove);
  saveConfig(function () {
    if (_.isFunction(cb)) {
      console.log('-----1-------->',  Matrix.config);
      cb();
    }
  });
}
...
function collectAppData(name, path, cb) {
  ...
  try {
    var config = yaml.safeLoad(fs.readFileSync(path + appDetectFile));
  } catch (e) {
    Matrix.loader.stop();
    return cb(err);
  }
  ...
  try {
    ...
    appDetails = {
      ...
      shortname: packageContent.shortname || name.toLowerCase().replace(" ", "_"),
