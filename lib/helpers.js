...
function displaySearch(raw, needle) {
  var table = new Table({
    chars: { 'mid': '', 'left-mid': '', 'mid-mid': '', 'right-mid': '' },
    head: [ t('matrix.helpers.search.name').underline, t('matrix.helpers.search.description') ],
    colWidths: [26, 54 + ttyExtra ]
  });
  ...
  _.each(_.values( raw ), function(app) {
    ...
    table.push([ app.name, app.desc.grey ]);
  })
...
function checkPolicy(config, cb) {
  ...
  var write = Matrix.helpers.configHelper.parsePolicyFromConfig( s );
